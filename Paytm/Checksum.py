...
def __pad_key__(key):
    # Ensure the key length is 32 bytes by padding or truncating it
    return key.ljust(32, '\0')[:32]  # Pad with '\0' or truncate if necessary

def __encode__(to_encode, iv, key):
    # Pad key to valid AES length
    key = __pad_key__(key)
    
    # Pad the string to be encrypted
